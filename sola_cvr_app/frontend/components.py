...
def _visitors_total(preview: pd.DataFrame) -> str:
    """
    Total Visitors that RECONCILES with the CVR: only count months that have
    Orders (matches the downloaded Excel TOTAL and the CVR denominator).
    Months with no Orders (N/A) are excluded, exactly like the Excel.
    """
    if "Orders" not in preview.columns or "Visitors" not in preview.columns:
        return _col_sum(preview, "Visitors")
    total = 0.0
    for _, r in preview.iterrows():
        o = str(r["Orders"]).strip()
        if o not in ("N/A", "", "nan", "None"):
            total += _parse_num(pd.Series([r["Visitors"]])).iloc[0]
    return f"{int(total):,}"
...
def render_metrics(platform: str, preview: pd.DataFrame, n_months: int):
    """
    Render context tiles only (Months + Period). No aggregate/TOTAL CVR is shown,
    because the visitor de-dup basis differs across platforms — a single overall
    CVR would not be apples-to-apples. Per-month CVR lives in the table.
    """
...
def render_table(platform: str, preview: pd.DataFrame):
    ...
    # so summing across months would not be apples-to-apples. Per-month only.
    st.markdown(
