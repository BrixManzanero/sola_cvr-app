    ...
    st.markdown(
        ...
        <div class="app-header">
          ...
          <div>
            ...
            <p class="header-sub">Shopee · TikTok · Lazada — Traffic Conversion Extractor</p>
          </div>
        </div>
        """,
...
def render_metrics(platform: str, preview: pd.DataFrame, n_months: int):
    ...
    if platform == "shopee":
        # Paid Order metrics: months, period, total orders, total clicks, overall CVR
        total_orders = _col_sum(preview, "Paid Orders")
        total_clicks = _col_sum(preview, "Product Clicks")
        # Overall CVR = total orders ÷ total clicks
        try:
            o = float(total_orders.replace(",", ""))
            c = float(total_clicks.replace(",", ""))
            overall_cvr = f"{o / c * 100:.2f}%" if c else "N/A"
        except Exception:
            overall_cvr = "N/A"
        cards = [
            ("Months",         str(n_months), ""),
            ("Period",         date_rng,      ""),
            ("Paid Orders",    total_orders,  ""),
            ("Product Clicks", total_clicks,  ""),
            ("Paid CVR",       overall_cvr,   "#2E5E0D"),
        ]
...
def render_table(platform: str, preview: pd.DataFrame):
    ...
    # CVR total per platform
    cvr_col = next((c for c in cols if "CVR" in c or c == "CVR"), None)
    if cvr_col:
        if platform == "shopee":
            num = sum(_num(v) for v in preview["Paid Orders"])
            den = sum(_num(v) for v in preview["Product Clicks"])
            totals[cvr_col] = f"{num / den * 100:.2f}%" if den else "N/A"
        elif platform == "tiktok":
            # CVR = Customers ÷ Visitors. Use Customers column if present;
            # otherwise fall back to visitor-weighted average of monthly CVRs.
            if "Customers" in preview.columns:
                num = sum(_num(v) for v in preview["Customers"])
                den = sum(_num(v) for v in preview["Visitors"])
                totals[cvr_col] = f"{num / den * 100:.2f}%" if den else "N/A"
            else:
                wsum = sum(_num(r[cvr_col]) * _num(r["Visitors"]) for _, r in preview.iterrows())
                vsum = sum(_num(r["Visitors"]) for _, r in preview.iterrows())
                totals[cvr_col] = f"{wsum / vsum:.2f}%" if vsum else "N/A"
        else:  # lazada
            num = sum(_num(v) for v in preview["Buyers"])
            den = sum(_num(v) for v in preview["Visitors"])
            totals[cvr_col] = f"{num / den * 100:.2f}%" if den else "N/A"
