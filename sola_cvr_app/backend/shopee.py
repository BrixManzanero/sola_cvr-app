...
def build_excel(monthly_df: pd.DataFrame, summaries: list) -> bytes:
    ...
    rows = []
    t_vis = 0.0      # only months that HAVE orders count toward the TOTAL,
    t_paid = 0.0     # so incomplete (visitor-only) months don't distort CVR.

    for _, row in monthly_df.iterrows():
        vis = _to_float(row.get("Visitors", 0))
        opaid = row.get("Paid Orders")
        has_orders = opaid is not None and not pd.isna(opaid)
        if has_orders:                   # month has Orders → include in TOTAL
            t_vis += vis
        ...
        rows.append([
            ...
            f"{int(round(_to_float(opaid))):,}" if has_orders else "N/A",
            cvr_str(opaid if has_orders else None, vis),
        ])

    write_plain_sheet(
        ws,
        headers=["Month", "Visitors", "Orders", "CVR"],   # 'Orders' = Paid Orders
        rows=rows,
        total_row=None,   # no aggregate TOTAL — visitor de-dup basis differs across platforms
        col_widths=[16, 14, 14, 12],
    )
