...
def _cvr_orders_over_visitors(sum_row: dict) -> str:
    """Unified CVR = Orders ÷ Visitors, as a clean '5.56%' string."""
    def _f(v):
        try:
            return float(str(v).replace(",", "").replace("%", "").strip())
        except Exception:
            return 0.0
    orders   = _f(sum_row.get("Orders", 0))
    visitors = _f(sum_row.get("Visitors", 0))
    return f"{orders / visitors * 100:.2f}%" if visitors else "N/A"
...
def extract(uploaded_file) -> dict:
    ...
    row = {
        ...
        # NEW unified CVR = Orders ÷ Visitors (was Lazada's native Buyers ÷ Visitors)
        "Conversion Rate": _cvr_orders_over_visitors(sum_row),
        "Native CVR":      fmt_cvr(sum_row.get("Conversion Rate", 0)),  # reference only
        "_day_count":      day_cnt,
...
def build_excel(monthly_df: pd.DataFrame, summaries: list) -> bytes:
    ...
    # Unified TOTAL CVR = total Orders ÷ total Visitors
    total_cvr = f"{t_ord / t_vis * 100:.2f}%" if t_vis else "N/A"
